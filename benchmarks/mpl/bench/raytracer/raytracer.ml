...
let render ~log_timings ~objs ~width ~height ~cam ~pool ~chunk_size =
  let n = height * width in
  let (output, tm) = Benchmark.getTime (fun _ -> Array.make n (0, 0, 0)) in
  if log_timings then log @@ sp "alloc_array %f" tm;
  let pixel l =
  ...
  in
  if log_timings then log @@ sp "compute_pixels %f" tm;
  {
    width;
    height;
    pixels = output
...
let () =
  let num_domains = Cla.parse_int "procs" 1 in
  let chunk_size_render = Cla.parse_int "chunk-size-render" 256 in

  let log_timings = Cla.parse_flag "log_timings" in
  let height = Cla.parse_int "m" 2000 in
  let width = Cla.parse_int "n" 2000 in
  let imgfile = getopt "-f" (Sys.argv |> Array.to_list) some None in
  let scene_name = Cla.parse_string "s" "rgbbox" in
  let scene =
  ...
  let ((objs, cam), tm) =
    Benchmark.getTime (fun _ ->
      Task.run pool (fun _ -> from_scene ~pool width height scene))
  in
  (* let t' = seconds() in *)
  ...
  let result = Benchmark.run "raytracer" bench in
