let usage_msg = "kernel2Seq_run.exe <sparse_graph_input_file> <samples_input_file"
let files = ref []
...
let () =
    ...
    ignore @@ Sys.opaque_identity @@ Array.map (fun start -> let g = graph in Kernel2Seq.kernel2 g start) samples;
