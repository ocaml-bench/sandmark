(* Graph 500 benchmark: Kernel 3

   Kernel 3 works on the Dijkstra's algorithm which is basically the shortest
   path from the node to all the nodes. As expected the main function here
   takes the HashMap and  the source node from where the shortest distnaces
...
(* Minimum Distance function computes the vertex which is at the min distance
 * from the node which is currently under study. Only the nodes which have not
 * been visited are only considered. This function is not checking the visited
 * as the verticesInspected already takes care of it in other function
 * (changeVerticeInspected). E.g. N1 is under study and now the nodes N2, N3
 * are at 2, inf distance in distanceArray, so N2 will be selected as the min
 * vertex. *)
...
(* After determining the min vertex (obtained from prev func), we have to
 * adjust dist such that, for all node, if dist[node] > dist[min vertex] +
 * weight_node_minVertex then update the distance else continue with other
 * nodes. Parent Array is also updated here.*)
...
(* This removes the nodes visited from the list verticesInspected *)
...
(* Dijkstra's algorithm which calculates the min vertex, then adjust the
 * distance, update the visited list and iterate till all nodes have been
 * inspected. *)
...
(* All intialisation done in the main function. Weights are float and vertices
 * are int *)
