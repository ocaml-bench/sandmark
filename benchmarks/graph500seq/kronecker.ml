...
let rec generateIIBitList list m ab =
  if List.length list = m then (List.rev list) else if Random.float (float_of_int(m)) > ab then generateIIBitList (1.::list) m ab else generateIIBitList (0.::list) m ab
;;

let rec generateJJBitList list ii_bit m a_norm c_norm =
  match ii_bit with 
    [] -> (List.rev list) |
    head::tail -> if Random.float (float_of_int(m)) > (c_norm *. (head) +. a_norm *. float_of_int (int_of_float(head) lxor 1)) 
    then generateJJBitList (1.::list) tail m a_norm c_norm else generateJJBitList (0.::list) tail m a_norm c_norm
;;
...
let rec modifyRowIJW kk_list index list newList = 
  match kk_list, list with
    ...
    _::_,[] -> [] | 
    [], _::_ -> [] |
    headII::tailII, headList::tailList -> let element = headList +. (( 2. **float_of_int(index) ) *. headII) in modifyRowIJW (tailII) index (tailList) (element::newList)
;;

let rec compareWithPr index m n ab a_norm c_norm ijw scale = 
  if index = scale then ijw else
    ...
    let ijw = ([firstRowIJW] @ [secondRowIJW] @ [List.nth ijw 2]) in compareWithPr (index+1) m n ab a_norm c_norm ijw scale
;; 

let permute list =
  let list = List.map (fun x -> (Random.bits (), x)) list in
  let list = List.sort compare list in List.map (fun x -> snd(x)) list
;;

let rec transpose col list newList = 
  if col = (List.length (List.nth list 0)) then newList else
    let rec adjustRow list newList col = 
      match list with 
        [] -> List.rev newList |
        head::tail -> adjustRow tail ((List.nth head col)::newList) col
    in transpose (col+1) list (newList@[adjustRow list [] col])  
;; 
...
let kronecker scale edgefactor = 
  ...
  let ijw = transpose 0 ijw [] in ijw
;;
