(*  Graph 500 benchmark: Kernel 2

    Kernel 2 aims at building a bfs tree where the bfs is maintained using a
    queue and the parent Array which consists of the immediate parent of the
    node.

    INPUTS : adjacency HashMap(for undirected graphs) and starting vertex which
    would allow you to begin the bfs *)
...
(* [appendVerticesToQueue] is the function doing recursion to push desired
 * elements in the queue, adjacent and ~visited nodes for a node. THe queue's
 * first element is always the node to be computed on for adjacency. As queue
 * pushs the node from front, head::tail helps directly, primrily pointing out
 * that queue has been considered as a list. *)
...
(* BFS function is a normal bfs function by building the bfs tree. Uses
 * adjacency HashMap, queue, parent Array, visited. bfsTree stores the order of
 * the nodes and it is a list. Parent array works as : parent[node] =
 * _parentNode_ is the DAT being used i.e. an array *)
...
(* Main function is the function where it calls bfs. Size computation is using
 * HashMap and the initialiszation of all arrays and lists happen here.*)
