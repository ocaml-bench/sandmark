...
let num_domains = try int_of_string Sys.argv.(2) with _ -> 5
...
let advance_ch bodies dt=
  ...
  let job2 i () =
    ...
    b.z <- b.z +. dt *. b.vz;
  in
  Array.iteri (fun i c -> C.send c.req (Do (job2 i))) channels ;
  Array.iter (fun c -> C.recv c.resp) channels
...
let () =
  ...
  offset_momentum_ch bodies;
  Printf.printf "%.9f\n" (energy_ch bodies);
  for i = 1 to n do advance_ch bodies 0.01 done;
  Printf.printf "%.9f\n" (energy_ch bodies)