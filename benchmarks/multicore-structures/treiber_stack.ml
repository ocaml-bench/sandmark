...
let rec add_node s n =
    let old_top = Atomic.get s.top in
        ...
        if Atomic.compare_and_set s.top old_top (Some n) then
            ()
        else
            add_node s n
...
let rec remove_node s =
    let old_head = Atomic.get s.top in
        if old_head = None then
            ...
            begin
                    match old_head with 
                    | None -> failwith "Internal inconsistency - got stack top with no value"
                    | Some(v) -> if Atomic.compare_and_set s.top old_head (Atomic.get v.next) then
                                    Some v.value
                                else
                                    remove_node s
            end
