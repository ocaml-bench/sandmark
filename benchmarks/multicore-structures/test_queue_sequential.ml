...
let () =
  ...
  for b = 0 to items_to_queue do
    ...
    | None ->
        failwith ("Lost an item at " ^ string_of_int b ^ "\n")
    | Some c ->
        item_sum := !item_sum - c
  done ;
  ...
  if !item_sum != 0 then
    ...
    Printf.printf
      "Queued and dequeued %d items in %f seconds = %f/ms per item\n"
      items_to_queue duration
      (1000.0 *. duration /. float_of_int items_to_queue)
