 ...
 * modified by KC Sivaramakrishnan: Remove parallelization
 *)
...
let dna =
  ...
  (try
     while true do
       let line = Bytes.unsafe_of_string (input_line kinput) in
...
let () =
  List.iter (fun i -> print_endline @@ write_frequencies i) [1; 2];
  List.iter (fun k -> print_endline @@ write_count k)
