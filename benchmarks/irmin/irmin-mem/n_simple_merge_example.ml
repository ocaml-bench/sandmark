...
let custom_clone src dst content key_list =
  ...
  let get_name x = counter := !counter + 1; (string_of_int counter.contents) ^ x in
  ...
  let rec custom_clone' src (dst : string) (content : string list) (key_list : string list) =
    ...
    | hd :: tl -> Store.clone ~src:src ~dst:dst >>= fun x ->
                  ...
                  custom_clone' src (get_name dst) tl key_list
...
let main () =
  ...
  update t ["root"; "misc"; "1.txt"] log_one >>= fun () ->
  custom_clone t "test" log_two ["root"; "misc" ; "sample.txt"] >>= fun () ->
  read_exn t ["root"; "misc"; "72_sample.txt"] >>= fun _ -> Lwt.return_unit
