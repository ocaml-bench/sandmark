...
for i in $(seq 0 $((COUNT - 1))); do
    ...
    for KIND in "sequential" "parallel" "perfstat" "pausetimes_seq" "pausetimes_par" ; do
        ...
        if [[ -n "${COMMIT}" ]] && $NOT_EXPIRED && ! is_old_commit "${KIND}" "${HOSTNAME}" "${COMMIT}"; then
            ...
            # Prepare run JSON
            TAG="${TAG_STRING}" make "${CONFIG_RUN_JSON}"

            # Build and execute benchmarks
            USE_SYS_DUNE_HACK=true SANDMARK_URL="${CONFIG_URL}" \
                RUN_CONFIG_JSON="${CONFIG_RUN_JSON}" \
                ENVIRONMENT="${CONFIG_ENVIRONMENT}" \
                OCAML_CONFIG_OPTION="${CONFIG_OPTIONS}" \
                OCAML_RUN_PARAM="${CONFIG_RUN_PARAMS}" \
                SANDMARK_CUSTOM_NAME="${CONFIG_NAME}" \
                SANDMARK_OVERRIDE_PACKAGES="${CONFIG_OVERRIDE_PACKAGES}" \
                SANDMARK_REMOVE_PACKAGES="${CONFIG_REMOVE_PACKAGES}" \
                RUN_BENCH_TARGET="${CONFIG_RUN_BENCH_TARGET}" \
                BUILD_BENCH_TARGET="${CONFIG_BUILD_BENCH_TARGET}" \
